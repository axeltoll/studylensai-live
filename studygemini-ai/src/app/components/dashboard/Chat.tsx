...
export default function Chat({
  ...
  useEffect(() => {
    const loadChatHistory = () => {
      try {
        ...
        const savedTypeHistory = localStorage.getItem(`studygemini_chat_history_${chatType}`);
        ...
        if (savedTypeHistory) {
          ...
          const globalHistory = localStorage.getItem(`studygemini_chat_history_global`);
          if (globalHistory) {
  ...
  useEffect(() => {
    const loadMessages = () => {
      if (currentChatId) {
        try {
          const savedMessages = localStorage.getItem(`studygemini_chat_${currentChatId}`);
          if (savedMessages) {
  ...
  const saveMessages = (chatId: string, messageList: Message[]) => {
    try {
      ...
      localStorage.setItem(`studygemini_chat_${chatId}`, JSON.stringify(messageList));
    } catch (err) {
  ...
  const updateChatHistory = (chatId: string, title: string, preview: string) => {
    ...
    try {
      ...
      localStorage.setItem(`studygemini_chat_history_${chatType}`, JSON.stringify(updatedHistory));
      
      // Also update the global history
      let globalHistory: any[] = [];
      const savedGlobalHistory = localStorage.getItem('studygemini_chat_history_global');
      ...
      localStorage.setItem('studygemini_chat_history_global', JSON.stringify(globalHistory));
    } catch (err) {
  ...
  const deleteChat = (chatId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (globalHistoryStr) {
        ...
        localStorage.setItem('studygemini_chat_history_global', JSON.stringify(updatedGlobalHistory));
      }
      
      // Remove chat messages
      localStorage.removeItem(`studygemini_chat_${chatId}`);
